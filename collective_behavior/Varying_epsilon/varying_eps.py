...
def run_simulation(speed_limit, eps, seed=None):
    ...
    for step in range(max_steps):
        ...
        for i, boid in enumerate(flock):
            ...
            informed = 1 if i < informed_boids else 0
            ...
            step_weights.append(wk)

        # Save metrics
        polarization.append(compute_polarization(flock))
        milling.append(compute_milling_metric(flock))
...
def run_all_simulations_for_eps(speed_limit, eps_val):
    ...
    for i in tqdm(range(N_simulations), desc=f"Running simulations for eps={eps_val}"):
        p, m, d, w = run_simulation(speed_limit, eps, seed=i)
        all_polarization.append(p)
        all_milling.append(m)
        all_distance.append(d)
        all_weights.append(w)
        print(f"Simulation {i} for $\\epsilon = {eps}$ done")
