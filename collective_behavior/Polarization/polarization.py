...
def update_weights(boid, neighbors, pi_u, dt, N_primitives, u_grid, speed_limit, radii, eps):
    ...
    log_det_cov_ratio = 0  # log(det(cov_q)/det(cov_p)) = 0

    # Use u_grid directly (shape: (M, 2))
    u_vecs = u_grid  # shape (M, 2)

    # Predict new velocities for each control input u_vec
    v_next = v_prev + u_vecs * dt
    ...
    target_vel = mean_vel # normalized vector
    ...
    kl_divs = 0.5 * (-dim + np.trace(inv_cov_q @ cov_p) + mahalanobis)  # shape (M,)

    # Uniform distribution over available controls
    q_u = np.ones(len(u_vecs)) / len(u_vecs)
    log_q_u = np.log(q_u)
...
dt = 0.05
area_limits = np.array([23, 23])    # must be equal
