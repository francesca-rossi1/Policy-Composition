...
def update_boid_cucker_smale(boid, neighbors, dt, speed_limit, beta=0.5):
    """
    Update an uninformed boid using the Cucker-Smale model.

    boid: Boid object
    neighbors: list of neighboring Boid objects
    dt: timestep
    speed_limit: max speed (used for clipping velocity norm)
    beta: decay exponent for interaction weight
    """
    norm_v = np.linalg.norm(boid.velocity)
    if norm_v > speed_limit:
        boid.velocity = boid.velocity / norm_v * speed_limit
    boid.position += boid.velocity * dt
    boid.trajectory.append(boid.position.copy())

    # Current velocity
    v_i = boid.velocity.copy()
    x_i = boid.position
    ...
    if neighbors:
        for n in neighbors:
            x_j, v_j = n.position, n.velocity
            d = np.linalg.norm(x_j - x_i)
            weight = 1.0 / ((1.0 + d ** 2) ** beta)
            dv += weight * (v_j - v_i)
    ...
    # Update velocity
    v_new = v_i + dv * dt
    boid.acceleration = dv

    # Clip velocity to speed limit
    norm_v = np.linalg.norm(v_new)
    if norm_v > speed_limit:
        v_new = v_new / norm_v * speed_limit
...
def run_step(frame_num):
    ...
    for i, boid in enumerate(flock):
        ...
        if boid in flock[:informed_boids]:
            ...
            cs_neighbors = boid.get_neighbors(flock, radius_cs)
            update_boid_cucker_smale(boid, cs_neighbors, dt, speed_limit, beta=0.4)
