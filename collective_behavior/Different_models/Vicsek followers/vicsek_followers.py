...
def update_boid_vicsek(boid, neighbors, dt, speed, eta):
    """
    Update an uninformed boid using the Vicsek model.

    boid: Boid object
    neighbors: list of neighboring Boid objects
    dt: timestep
    speed: fixed speed for all boids
    eta: width of the uniform noise interval (radians)
    """
    # Update position
    boid.position += boid.velocity * dt

    if neighbors:
        # Get average direction of neighbors
        angles = [np.arctan2(n.velocity[1], n.velocity[0]) for n in neighbors]
        mean_angle = np.arctan2(np.mean(np.sin(angles)), np.mean(np.cos(angles)))
    else:
        # If no neighbors, keep current direction
        mean_angle = np.arctan2(boid.velocity[1], boid.velocity[0])

    # Add random noise in range [-eta/2, eta/2]
    noise = np.random.uniform(-eta/2, eta/2)
    new_angle = mean_angle + noise

    # Update velocity with fixed speed
    boid.velocity = speed * np.array([np.cos(new_angle), np.sin(new_angle)])
...
def run_step(frame_num):
    ...
    for i, boid in enumerate(flock):
        ...
        if boid in flock[:informed_boids]:
            ...
            v_neighbors = boid.get_neighbors(flock, radius_vicsek)
            update_boid_vicsek(boid, v_neighbors, dt, speed_vicsek, eta=0.5)
...
# Initial conditions
np.random.seed(1)
positions = (np.random.rand(num_boids, 2)-0.5) * 3
...
# --- enforce constant speed ONLY for Vicsek boids ----
vicsek_indices = np.arange(informed_boids, num_boids)
speed_vicsek = speed_limit
# Normalize all Vicsek velocities to exactly speed_limit
norms = np.linalg.norm(velocities[vicsek_indices], axis=1, keepdims=True)
# avoid division by zero
norms = np.where(norms == 0, 1e-8, norms)
velocities[vicsek_indices] = velocities[vicsek_indices] / norms * speed_vicsek
# Uniform weights
