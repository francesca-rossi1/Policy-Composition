...
def update_boid_couzin(boid, neighbors, dt, speed_limit,
                       r_r, r_o, r_a, noise=0.05):
    x_i, v_i = boid.position, boid.velocity
    repulse_vecs, orient_vecs, attract_vecs = [], [], []
    # Classify neighbors
    for n in neighbors:
        dvec = n.position - x_i
        ...
        if dist < 1e-8:
            continue
        if dist < r_r:
            repulse_vecs.append(-dvec / dist)
        elif dist < r_o:
            orient_vecs.append(n.velocity / (np.linalg.norm(n.velocity) + 1e-8))
        elif dist < r_a:
            attract_vecs.append(dvec / dist)
    # Decide desired direction
    if repulse_vecs:
        desired = np.mean(repulse_vecs, axis=0)
    else:
        have_o, have_a = len(orient_vecs) > 0, len(attract_vecs) > 0
        if have_o and have_a:
            d_o, d_a = np.mean(orient_vecs, axis=0), np.mean(attract_vecs, axis=0)
            desired = 0.5 * (d_o / np.linalg.norm(d_o) + d_a / np.linalg.norm(d_a))
        elif have_o:
            desired = np.mean(orient_vecs, axis=0)
        elif have_a:
            desired = np.mean(attract_vecs, axis=0)
        else:  # no neighbors
            desired = v_i / np.linalg.norm(v_i)
    # Normalize desired
    if np.linalg.norm(desired) > 1e-8:
        desired /= np.linalg.norm(desired)
    else:
        desired = v_i / (np.linalg.norm(v_i) + 1e-8)
    # Add angular noise
    theta = np.arctan2(desired[1], desired[0])
    theta += np.random.uniform(-noise, noise)
    # Current heading
    theta_i = np.arctan2(v_i[1], v_i[0])
    # Shortest angular difference
    dtheta = np.arctan2(np.sin(theta - theta_i), np.cos(theta - theta_i))
    omega_max = np.radians(120)
    dtheta = np.clip(dtheta, -omega_max * dt, omega_max * dt)
    # Update position
    boid.position += boid.velocity * dt
    boid.trajectory.append(boid.position.copy())
    # Update velocity
    theta_new = theta_i + dtheta
    new_vel = np.array([np.cos(theta_new), np.sin(theta_new)]) * speed_limit
...
def run_step(frame_num):
    ...
    for i, boid in enumerate(flock):
        ...
        if boid in flock[:informed_boids]:
            informed = 1
            wk = update_weights(boid, neighbors, primitives, dt, N_primitives, u_grid, speed_limit, radii, informed,
                                eps)
            steering = compute_steering(primitives, wk, u_grid, u_max_norm)
            update_boid(boid, wk, steering, dt, speed_limit)
        else:
            update_boid_couzin(boid, neighbors, dt, speed_limit, radii['sep'], radii['ali'], radii['coh'])
...
velocities = (np.random.rand(num_boids, 2) - 0.5) * 1
# Compute norms
